...
return new class extends Migration
{
    ...
    {
        DB::unprepared(<<<'SQL'
        DROP FUNCTION IF EXISTS academia.fn_log_bitacora;
        ...
        DECLARE
          v_col_persona TEXT;
        BEGIN
          IF to_regclass('academia.bitacora') IS NULL THEN
            RETURN;
          END IF;
          ...
          IF v_col_persona = 'persona_id' THEN
            EXECUTE 'INSERT INTO academia.bitacora(persona_id, rol, modulo, accion, detalle, estado, payload, extra, created_at)
                     VALUES ($1,$2,$3,$4,$5,$6,$7,$8, now())'
            USING p_persona, p_rol, p_modulo, p_accion, p_detalle, p_estado, p_payload, p_extra;
          ELSIF v_col_persona = 'persona' THEN
            EXECUTE 'INSERT INTO academia.bitacora(persona, rol, modulo, accion, detalle, estado, payload, extra, created_at)
                     VALUES ($1,$2,$3,$4,$5,$6,$7,$8, now())'
            USING p_persona, p_rol, p_modulo, p_accion, p_detalle, p_estado, p_payload, p_extra;
          END IF;
        EXCEPTION WHEN undefined_column THEN
          -- evitar que falle la transacción si la estructura es distinta
          NULL;
        END;
        $$;
        SQL);
